...
### ADD NEW COLUMN    #######################################################################################  
# cursor.execute("ALTER TABLE LongTail.ParseContent ADD textSpinned longtext")
...
driver.find_element_by_xpath("/html/body/div[1]/div/div/main/article/div/div/div[1]/div/div/div[2]/div/div[1]/div[2]/div[1]/div/span/span[1]/span/span[1]").click()
...
driver.find_element_by_xpath("/html/body/div[1]/div/div/main/article/div/div/div[1]/div/div/div[2]/div/div[1]/div[2]/div[2]/div/span/span[1]/span/span[1]").click()
driver.find_element_by_class_name("select2-search__field").send_keys("Spanish")
driver.find_element_by_class_name("select2-search__field").send_keys(Keys.ENTER)
...
while i <= 10500:
    ...
    print("comienza pegado de texto en page")
    driver.find_element_by_xpath('//*[@id="inp"]').send_keys(sentences)
    print("cesperando que cargue el texto")
    wait1 = WebDriverWait(driver, 240).until(EC.visibility_of_element_located((By.ID, 'submit-spinner')))
    print("clic en boton paraphrase")
    ...
    cursor.execute("UPDATE LongTail.ParseContent SET textSpinned = %s WHERE id_Key= %s",(contents,i))                
    cursor.execute("SELECT parrafos FROM LongTail.ParseContent WHERE id_Key= %s",(i))
    myresult1 = cursor.fetchone()
    
    db.commit() 
    
    driver.find_element_by_id("inp").clear()
    driver.find_element_by_id('out').clear()
